...
\begin{itemize}
  ...
  \item A \texttt{SoftwareSerial} amostra os bits por temporização e é sensível a atrasos no atendimento de interrupções. Erros ocasionais de leitura são detectados e corrigidos pelo protocolo, mas cada um consome uma das três tentativas;
...
Os cenários do menu também foram executados no Tinkercad, com os dois Arduinos ligados conforme a Tabela~\ref{tab:ligacao}. Além das falhas simuladas, surgiram erros reais ocasionais na leitura das respostas, que o protocolo detectou e corrigiu. No registro abaixo, ao enviar dados com a opção de perda do ACK, a segunda tentativa recebeu \texttt{0x06 0xDA} em vez de \texttt{0x06 0x5A} (sequência 90):
