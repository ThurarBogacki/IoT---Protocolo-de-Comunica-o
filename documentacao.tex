...
\midrule
\texttt{BAUD\_RATE} & 2400 bps (8N1) & TX e RX & Taxa das portas serial e \texttt{SoftwareSerial} \\
\texttt{TAMANHO\_MAX\_DATA} & 64 bytes & TX e RX & Tamanho máximo do \emph{payload} de dados variáveis \\
\texttt{MAX\_TENTATIVAS} & 5 & TX & Número máximo de envios de uma mesma mensagem \\
...
Cada quadro possui 6 bytes de controle além do \emph{payload}. Na configuração 8N1, cada byte ocupa 10 bits na linha (início, 8 bits de dados e parada); portanto, a 2400 bps, cada byte leva aproximadamente 4,17~ms para ser transmitido. A Tabela~\ref{tab:tempos} apresenta o tamanho dos quadros e os tempos teóricos de transmissão.

\begin{table}[!htb]
\centering
\caption{Tamanho dos quadros e tempo teórico de transmissão a 2400 bps.}
\label{tab:tempos}
\small
\begin{tabular}{lcccc}
\toprule
...
\midrule
\emph{byte} & 1 byte & 7 bytes & 29,2 ms & 37,5 ms \\
\emph{word} & 2 bytes & 8 bytes & 33,3 ms & 41,7 ms \\
\emph{float} & 4 bytes & 10 bytes & 41,7 ms & 50,0 ms \\
dados & 1--64 bytes & 7--70 bytes & 29,2--291,7 ms & 37,5--300,0 ms \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item O transmissor aguarda o envio das mensagens pendentes ao Monitor Serial (\texttt{Serial.flush()}); em seguida, o quadro é montado, o CRC-8 é calculado e todos os bytes são transmitidos;
  \item O transmissor aguarda os dois bytes da resposta por até 500~ms, contados a partir do fim da transmissão do quadro;
  ...
  \item Após cinco tentativas sem confirmação, a função retorna \texttt{false}.
\end{enumerate}
...
Quando todas as verificações são satisfeitas, o receptor envia o ACK \emph{antes} de apresentar os dados no Monitor Serial. Essa ordem é importante porque a escrita de uma mensagem longa no Monitor Serial a 2400 bps pode bloquear o programa por dezenas de milissegundos, tempo que seria descontado do \emph{timeout} do transmissor. Os dados são apresentados de acordo com o tipo identificado, como nos exemplos:
...
Receptor ausente ou desconectado & Cinco \emph{timeouts} consecutivos & Falha informada ao usuário \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
  ...
  \item A \texttt{SoftwareSerial} amostra os bits por temporização e é sensível a atrasos no atendimento de interrupções. A taxa de 2400 bps reduz essa sensibilidade à custa da velocidade, e eventuais erros de leitura são detectados e corrigidos pelo protocolo, consumindo uma das cinco tentativas;
...
\emph{Stop-and-Wait} com ACK e NACK & Simples e com pouco uso de memória; adequado à interação do usuário, que envia uma mensagem por vez. O NACK acelera a recuperação (cerca de 90~ms em quadros curtos, contra 500~ms de \emph{timeout}) \\
...
\emph{Timeout} de resposta de 500~ms & A contagem começa após o envio do quadro; a resposta mais demorada, um NACK enviado após 50~ms de descarte, chega em cerca de 58~ms, o que deixa ampla margem sem tornar a detecção de falhas lenta \\
\emph{Timeout} entre bytes de 50~ms & Cerca de 12 vezes o tempo de um byte, tolerando atrasos sem demorar a rejeitar quadros incompletos \\
Taxa de 2400 bps & A 4800 bps, o Tinkercad apresentou erros ocasionais de leitura (Seção~\ref{sec:resultados}). A 2400 bps cada bit dura o dobro do tempo, o que dobra a tolerância a atrasos na amostragem; nenhum erro foi observado em testes repetidos. O custo é o dobro do tempo de transmissão (Tabela~\ref{tab:tempos}) \\
Até cinco tentativas & Uma falha simulada consome uma tentativa; com cinco, erros ocasionais adicionais não levam a uma falha definitiva. O custo é uma espera maior para informar a ausência do receptor: cerca de 3,4~s para um \emph{byte} e 4,8~s para 64 bytes \\
Monitor Serial esvaziado antes de cada transmissão & A \texttt{SoftwareSerial} amostra os bits por temporização; evitar interrupções da porta serial nativa durante a troca de quadros reduz erros na leitura da resposta \\
...
2 & Corromper todas as tentativas & Cinco NACKs e falha definitiva \\
...
Os testes foram executados em um ambiente de simulação em C++ construído para o trabalho. Nele, os dois programas (\texttt{Transmissor.cpp} e \texttt{Receptor.cpp}) são compilados sem alterações, com implementações simplificadas das bibliotecas do Arduino, e executados simultaneamente, conectados por um canal serial emulado. O canal reproduz o tempo de transmissão de cada byte a 2400 bps (cerca de 4,17~ms) e a capacidade dos \emph{buffers} de recepção do Arduino, além de permitir a injeção de falhas físicas que não dependem do menu: desconexão do receptor, perda de um byte e corrupção da resposta. As entradas do Monitor Serial foram roteirizadas, e os programas foram compilados sem avisos com as opções \texttt{-Wall} e \texttt{-Wextra} do GCC. Como a escrita no Monitor Serial é instantânea no simulador, os tempos medidos não incluem a espera por \texttt{Serial.flush()}.

Os cenários cobrem os quatro tipos de dados, valores nos limites dos intervalos, todas as opções da Tabela~\ref{tab:simulacao}, falhas físicas na linha e entradas inválidas do usuário. Os cenários baseados no menu também foram executados no Tinkercad, cujas observações são apresentadas ao final da Seção~\ref{sec:resultados}.
...
\midrule
1 & \emph{Byte} 150, sem falha & ACK na 1.ª tentativa; RX exibe 150 & Sucesso & 39 ms \\
2 & \emph{Float} 3,1415 e $-12{,}5$, sem falha & ACK na 1.ª tentativa; RX exibe 3.1415 e -12.5000 & Sucesso & 52 ms \\
3 & \emph{Byte} 170 (\texttt{0xAA} no \emph{payload}) & ACK na 1.ª tentativa; RX exibe 170 & Sucesso & 40 ms \\
4 & \emph{Word} 65535 (valor máximo) & ACK na 1.ª tentativa; RX exibe 65535 & Sucesso & 43 ms \\
5 & \emph{Word} 1024, corromper a 1.ª (opção 1) & RX: CRC inválido e NACK; ACK na 2.ª tentativa & Sucesso & 336 ms \\
6 & \emph{Float} 3,1415, corromper todas (opção 2) & Cinco NACKs; RX não exibe dados & Falha & 1310 ms \\
7 & Dados ``IoT'', perder o quadro (opção 3) & \emph{Timeout} na 1.ª; ACK na 2.ª tentativa & Sucesso & 747 ms \\
8 & Dados com 64 bytes, perder o ACK (opção 4) & ACK ignorado; na 2.ª tentativa RX reconhece a duplicata e exibe a mensagem uma única vez & Sucesso & 818 ms \\
9 & Receptor desconectado & Cinco \emph{timeouts} & Falha & 3451 ms \\
10 & Byte perdido no meio de um quadro de 10 bytes & RX: tamanho incompatível e NACK; ACK na 2.ª tentativa & Sucesso & 406 ms \\
11 & ACK corrompido na linha (\emph{word} 42) & Resposta inválida; na 2.ª tentativa RX reconhece a duplicata & Sucesso & 287 ms \\
12 & Bytes espúrios com \texttt{0xAA} antes de um quadro & RX rejeita o falso quadro; TX descarta o NACK atrasado e entrega na 1.ª tentativa & Sucesso & 43 ms \\
\bottomrule
\end{tabularx}
\end{table}

Os tempos medidos são coerentes com a análise teórica. No cenário~1, o valor de 39~ms se aproxima dos 37,5~ms previstos na Tabela~\ref{tab:tempos}. No cenário~5, a primeira tentativa é rejeitada após cerca de 92~ms (33,3~ms do quadro, 50~ms de descarte e 8,3~ms do NACK), seguidos de 200~ms de espera e de uma nova transmissão bem-sucedida. No cenário~9, o tempo de 3451~ms corresponde a cinco esperas de 500~ms somadas aos quadros e aos quatro intervalos de 200~ms, próximo dos 3446~ms previstos. Esse é o tempo máximo que o usuário aguarda antes de ser informado de uma falha no envio de um \emph{byte}; para dados de 64 bytes, o limite teórico é de cerca de 4,8~s. A comparação entre os cenários~5 e~7 mostra a vantagem do NACK: a recuperação por NACK foi mais de 400~ms mais rápida que a recuperação por \emph{timeout}.
...
\subsection*{Execução no Tinkercad}

Os cenários do menu também foram executados no Tinkercad, com os dois Arduinos ligados conforme a Tabela~\ref{tab:ligacao}. Na configuração inicial, com 4800 bps e até três tentativas, surgiram, além das falhas simuladas, erros reais ocasionais na leitura das respostas, que o protocolo detectou e corrigiu. No registro abaixo, obtido nessa configuração, ao enviar dados com a opção de perda do ACK, a segunda tentativa recebeu \texttt{0x06 0xDA} em vez de \texttt{0x06 0x5A} (sequência 90):

\begin{small}
\begin{verbatim}
[TX] Tentativa 1/3 (seq 90)
[SIMULACAO] ACK perdido na linha (tratado como timeout)
[TX] Tentativa 2/3 (seq 90)
[TX] Resposta invalida do receptor: 0x6 0xDA
[TX] Tentativa 3/3 (seq 90)
[TX] ACK recebido
[RESULTADO] SUCESSO: mensagem confirmada pelo receptor
\end{verbatim}
\end{small}

Os valores \texttt{0x5A} (\texttt{01011010}) e \texttt{0xDA} (\texttt{11011010}) diferem apenas no bit mais significativo, que é o último bit de dados transmitido, imediatamente antes do bit de parada (sempre 1). O byte foi amostrado com atraso, e o bit de parada foi lido no lugar do último bit de dados. Esse comportamento é característico de portas seriais implementadas em software, cuja amostragem depende do atendimento imediato das interrupções e da precisão de laços de espera. Como o número de sequência não conferiu, a resposta foi descartada e a mensagem, retransmitida, sendo entregue na terceira tentativa sem que o receptor exibisse os dados duas vezes. O mesmo tipo de ocorrência foi observado com a opção de perda do quadro, também concluída com sucesso na terceira tentativa.

Com base nessa análise, a taxa de transmissão foi reduzida para 2400 bps, o que dobra a duração de cada bit e, com ela, a margem de amostragem, e o número máximo de tentativas foi ampliado para cinco. Nessa configuração final, os cenários foram repetidos várias vezes no Tinkercad sem nenhum erro de leitura.
...
\midrule
Comunicação entre dois Arduinos por porta serial & \texttt{SoftwareSerial} nos pinos 10 e 11 a 2400 bps & \ref{sec:protocolo} \\
...
Retransmitir mensagens quando necessário & \emph{Stop-and-Wait} ARQ com até cinco tentativas & \ref{sec:confiabilidade} \\
